        ...
        <fieldset class="question-card">
          ...
          ${question.options
            .map(
              (option, optionIndex) => `
                <label class="option">
                  <input type="radio" name="${question.id}" value="${optionLetter(optionIndex)}" />
                  <span>${optionLetter(optionIndex)}. ${option}</span>
                </label>`
            )
            .join("")}
        </fieldset>`
        ...
        <div class="question-card">
          ...
          <div class="blank-inputs">
            ${Array.from({ length: question.blanks }, (_, blankIndex) => {
              const label = question.blanks === 1 ? "Answer" : `Blank ${blankIndex + 1}`;
              return `<label>${label}<input name="${question.id}_${blankIndex + 1}" /></label>`;
            }).join("")}
          </div>
        </div>`
    )
      ...
      (question, index) => `
        <div class="question-card">
          <label class="essay-toggle">
            <input type="checkbox" class="essay-select" value="${question.id}" />
            <span>Answer Question ${index + 1}: ${question.title}</span>
          </label>
          <p>${question.prompt.replace(/\n/g, "<br />")}</p>
          <textarea name="${question.id}" disabled placeholder="Select this essay question before typing your answer."></textarea>
        </div>`
    )
    .join("");
}
...
function getSelectedEssays() {
  return [...document.querySelectorAll(".essay-select:checked")].map((checkbox) => checkbox.value);
}

function updateEssayControls() {
  const selected = getSelectedEssays();
  document.querySelectorAll(".essay-select").forEach((checkbox) => {
    const textarea = document.querySelector(`textarea[name="${checkbox.value}"]`);
...
function collectPayload() {
  ...
  const answers = {};
  exam.mcq.forEach((question) => {
    answers[question.id] = data.get(question.id) || "";
  });

  const fillBlanks = {};
  exam.blanks.forEach((question) => {
    fillBlanks[question.id] = Array.from({ length: question.blanks }, (_, index) =>
      String(data.get(`${question.id}_${index + 1}`) || "").trim()
    );
  });

  const essays = getSelectedEssays().map((id) => {
    const question = exam.essays.find((item) => item.id === id);
    return {
      id,
...
function updateSummary() {
  ...
  const essayWords = payload.essays.reduce((sum, essay) => sum + essay.answer.split(/\s+/).filter(Boolean).length, 0);
  ...
  if (railProgress) railProgress.style.width = `${Math.max(4, progress)}%`;
...
function validateEssays(payload) {
  if (payload.essays.length !== 4) return "Please select exactly four essay questions in Section C.";
  if (payload.essays.some((essay) => !essay.answer)) return "Please write an answer for each selected essay question.";
  return "";
}

function validateExamSections(payload) {
...
form.addEventListener("submit", async (event) => {
  ...
  const payload = collectPayload();
  const examError = validateExamSections(payload);
  if (!form.reportValidity() || examError) {
    statusMessage.textContent = examError || "Please complete all required fields before submitting.";
    statusMessage.classList.add("error");
    return;
  }
